...
- [ ] 4.1 Split `main.tsx` god component — `packages/client/src/main.tsx:108-442` `App()` mixes route branch + data load + 12 `useState` + shell cache refs. Extract `useAppRouteLoader()` (loadPage, catalog, shell cache) + `useAppRoute()` (resolveRoute, editMode, redirects), keep `App` thin. Accept: `main.tsx` App under ~120 lines, home/login/admin render unchanged, 0 console errors.
- [ ] 4.2 Split `use-edit-page-orchestration` — `packages/client/src/editor/hooks/use-edit-page-orchestration.ts:40-604` (562 lines). Split into `useEditorRoute` / `useCatalogLoader` / `useEditorMutations` / `useCanvasDnd`. Accept: no file over ~200 lines, `?edit=true` editor loads + saves, 0 errors.
- [ ] 4.3 Fix `EditorCanvas` remount — `packages/client/src/editor/components/canvas/EditorCanvas.tsx:539` `key={specStructureKey(previewSpec)}` remounts preview shell per keystroke. Drop remount key, memo leaf blocks. Accept: typing in PropsPanel no longer remounts canvas (React DevTools highlight stable), 0 errors.
- [ ] 4.4 Replace per-keystroke `JSON.stringify` equality — `editor/collab/automerge-spec.ts:176,191,196,209`, `editor/collab/use-layout-collab.ts:26`, `editor/lib/spec-utils.ts:16` (clone is fine, keep). Use existing `diffToPatches`/`applySpecPatch` + field dirty flags like `use-content-draft.ts` should. Accept: no `JSON.stringify` in hot compare paths, typing latency down, 0 errors.
- [ ] 4.5 `React.memo` pass — zero `memo` in client today. Memoize canvas leaves + palette rows + layer tree rows; split `editor-session.tsx:72-73` + `use-editor-prefs` single contexts so edits don't re-render shell. Accept: keystroke re-renders scoped to edited block (profiler), 0 errors.
- [ ] 4.6 Single `ROUTE_TABLE` — three registries: `platform-routes.ts` (platform/storefront), `auth/admin-routes.ts` (sidebar/nav), `auth/account-routes.ts` (account mirror). Merge into one table in `platform-routes.ts`, others re-export/derive. Accept: one source of truth, `/admin/*` + `/account/*` + `/login` all resolve + highlight correctly, 0 errors.
- [ ] 4.7 Reverse editor→admin imports — `editor/content-fields.ts:2-3` re-exports admin inputs; `platform/registry.ts:3`, `platform/catalog.ts:4`, `platform/admin-platform-view.tsx:9` import from `../admin`. Move shared inputs to `components/shared`, platform imports from there, admin keeps its own. Accept: no `editor/*` → `admin/*` and no `platform/*` → `admin/*` imports (`grep` clean), all forms render, 0 errors.
- [ ] 4.8 Route client `fetch` via `lib/api.ts` — raw `fetch` in `registry.ts:26` (`POST /api/machines/cart/add`), `auth/org.ts:20`, `auth/idp-login.ts:33,58`, `auth/config.ts:13`, `core/actions/auth.ts:39`. Migrate all to `lib/api.ts` `apiFetch` (HMAC/JWT/org headers in one place). Accept: no raw `fetch` outside `lib/api.ts` (`grep` clean), login/cart/catalog flows work, 0 errors.
- [ ] 4.9 Code-split TipTap/Yjs/Automerge — `package.json:14-17,32-44,50,55` + `rspack.config.mjs:108-124`: TipTap ×10, Yjs, Automerge ×3, `rrweb-player` ship in initial bundle. `React.lazy` the rich-text editor + collab hooks (`RichTextTipTapEditor.tsx:10-18`, `use-rich-text-collab.ts:3-5`), `cacheGroups` for tiptap/automerge/yjs, import `automerge/slim` only. Accept: initial JS down on storefront (no editor libs on `/`), editor still loads on demand, 0 errors.
- [ ] 4.10 Fix client `tsc` debt — dual `@json-render` 0.19 vs 0.20 (`catalog-loader.ts:92`, `registry.ts:15-21`) + `catalog.ts:12` `slots` type gap. Pin single 0.20.x across client, fix `slots` prop typing. Accept: `pnpm --filter @noname/client exec tsc --noEmit` clean.
