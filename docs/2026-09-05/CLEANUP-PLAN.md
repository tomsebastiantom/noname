...
- [x] 4.1 Split `main.tsx` god component — done: `platform/use-app-route.ts` (route resolution) + `platform/use-app-page-loader.ts` (all loading state + catalog/schema fetch), `App` is render branch only. Verified live: home + product, 0 errors. Follow-up fix: hook returned an inline loadPage wrapper (new identity per render) which retriggered the load effect forever -- hundreds of catalog/schema/flags calls on login. Fixed with stable useCallback + verified 10s idle = 1 catalog + 1 schema.
