...
|---------|--------|-------|
| `server` | ✅ Full DDD, 8 domains | Hono + Drizzle + XState + ClickHouse + BullMQ + OpenTelemetry |
| `browser-sdk` | ✅ 6 modules, Datadog RUM parity | Vanilla TS, Vite library mode. 5.26 KB gzipped core |
| `workers` | ✅ Routes + cache + auth stub | Hono + CF Workers. SSR pending |
| `client` | ✅ Scaffold + catalog + MF loader | React 19 + json-render + Module Federation. Rspack build |
...
| Logto auth integration | x-tenant-id header sufficient |
