...
| 8 | ~~Add alerting on the event-bus Redis-fallback path~~ — **Done, 2026-08-07.** New `shared/redis-fanout-status.ts`: `event-bus.ts`, `sse-manager.ts`, and `collab-redis-relay.ts` (item 7) all now `console.error` on the connect failure (was silently swallowed) and self-register with a shared registry. `/health` now returns `{ redis: { degraded, subsystems } }` for dashboards/uptime checks to poll — kept at HTTP 200 deliberately so a Redis blip doesn't trip liveness probes and restart an otherwise-healthy replica. A 5-minute heartbeat (`startRedisFanoutMonitor`) re-logs at ERROR level while any subsystem stays degraded, since a single log line at the moment of failure is exactly the kind of thing that scrolls out of most retention windows before anyone's paging rule fires on it. | `SCALABILITY.md` |
| 9 | ~~Fix N+1s~~ — **Done, 2026-08-07.** Zitadel has no batch "authentication factors for N users" endpoint, so `auth/service.ts`'s `listTeamUsers` now fans out via a new bounded-concurrency helper (`shared/concurrency.ts`'s `mapWithConcurrency`, limit 10) instead of an unbounded `Promise.all` — bounded so a large team doesn't trip Zitadel's own rate limit. `scope/service.ts`'s `revokeTuples` switched from a sequential `for` loop to `Promise.all` (Keto tuple deletes are independent, no batch-delete-by-list endpoint exists either). `flags/service.ts`'s hot evaluation path now does one bulk insert per `evaluate()` call via a new `FlagStorage.recordEvaluations`/`evaluation.ts`'s `recordEvaluations`, instead of one `INSERT` per flag; `evaluateBatch` picks this up for free since it calls `evaluate()` per context. | `SCALABILITY.md` |
| 10 | ~~Separate background workers from the API process~~ — **Done, 2026-08-07.** New `shared/worker-runtime.ts`: every domain's `startXWorker` (agent, both webhook queues, notifications/email, tenant catalog, analytics) now checks `workersEnabled()` (`RUN_WORKERS`, default `true`) and returns `null` before constructing a BullMQ `Worker` when disabled, and reads its concurrency from a per-queue env var (e.g. `AGENT_WORKER_CONCURRENCY`) instead of a hardcoded constant. Extracted the domain-wiring that used to live in `index.ts` into `bootstrap.ts`'s `createApp()`, and added a new `worker.ts` entrypoint that calls the same `createApp()` but never calls `serve()` — so it builds the same domain graph (and thus starts the same workers) without binding a port. Run API replicas with `RUN_WORKERS=false` + `worker.ts` replicas (default `RUN_WORKERS=true`) to get independent HTTP/job-processing scaling and separate DB connection pools, without the larger refactor of splitting routes out of each domain factory. | `SCALABILITY.md` |
