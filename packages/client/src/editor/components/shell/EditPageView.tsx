...
import { releaseEditorDevtools } from "../../activate-editor-devtools";
...
import { useEditorShell } from "../../hooks/use-editor-shell-labels";
import { useLayoutDraft } from "../../hooks/use-layout-draft";
import {
  addComponentToSpec,
  canRemoveElement,
...
import { defaultPropsForType } from "../palette/ComponentPalette";
...
export function EditPageView({
  ...
  useEffect(() => () => releaseEditorDevtools(), []);
  ...
  const session = useMemo((): EditorSessionValue | null => {
  ...
  if (shellLabelsMissing || !shellLabels || !session || !mergedShellSpec) {
    return (
      <p className="p-8 text-destructive">
        Editor chrome labels missing — seed or publish the <code>visual_editor</code> layout (
        <code>VisualEditorShell.props.labels</code>).
      </p>
    );
  }

  if (loading && !storedSpec) {
    return <p className="p-8 text-muted-foreground">{shellLabels.loadingLayoutHint}</p>;
  }

  return (
    <EditorSessionProvider value={session}>
      <EditorPrefsProvider templateName={templateName} registry={registry}>
        <JSONUIProvider registry={editorRegistry} store={editorShellStore} handlers={{}}>
