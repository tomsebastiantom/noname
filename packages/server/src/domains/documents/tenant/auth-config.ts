import { isSupportedLoginProvider } from "../content-types/auth-provider";
import type { MediaRef, TenantAuthConfig } from "../ports";
...
/** Normalize tenant settings `auth` JSON — icons use the same MediaRef shape as content fields. */
export function normalizeAuthConfig(raw: unknown): TenantAuthConfig {
  ...
  const providerIconAssets: Record<string, MediaRef> = {};
  if (
