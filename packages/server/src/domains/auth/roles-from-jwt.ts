...
/** Parse platform role keys from a ZITADEL JWT payload. */
...
export function teamRoleFromJwt(
  ...
  const primary = primaryTeamRole(rolesFromJwt(payload, options));
  if (primary === "admin" || primary === "editor") return primary;
  return null;
}
