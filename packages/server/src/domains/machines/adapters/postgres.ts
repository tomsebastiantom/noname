  ...
  return {
    async findDefinition(tenantId, name) {
      const [row] = await db
        ...
        .where(and(eq(machineDefinitions.tenantId, tenantId), eq(machineDefinitions.name, name)));
    ...
    async listDefinitions(tenantId) {
      const rows = await db
        .select()
        .from(machineDefinitions)
        .where(eq(machineDefinitions.tenantId, tenantId));
    ...
    async findInstance(tenantId, id) {
      const [row] = await db
        ...
        .where(and(eq(machineInstances.tenantId, tenantId), eq(machineInstances.id, id)));
    ...
    async updateInstance(instance) {
      const [row] = await db
        ...
        .where(
          and(
            eq(machineInstances.tenantId, instance.tenantId),
    ...
    async listInstances(tenantId) {
      const rows = await db
        .select()
        .from(machineInstances)
        .where(eq(machineInstances.tenantId, tenantId));
