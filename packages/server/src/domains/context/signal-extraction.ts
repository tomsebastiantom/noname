...
export function extractSignals(headers: Record<string, string>): ContextSignal[] {
  ...
  else if (/iphone|ipad|mac os/i.test(ua)) signals.push({ category: "device", key: "os", value: "ios" });
  ...
  if (country) signals.push({ category: "geography", key: "country", value: country.toUpperCase() });

  // User tier (from cookie, if present)
  const cookie = h["cookie"] || "";
  const tierMatch = cookie.match(/tier=([a-z]+)/i);
  if (tierMatch) signals.push({ category: "user", key: "tier", value: tierMatch[1]!.toLowerCase() });
