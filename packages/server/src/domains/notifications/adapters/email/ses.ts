  ...
  return {
    async send(credentials: CommsCredentials, input: SendEmailInput & { from?: string }) {
      if (credentials.provider !== "ses") {
        throw new ServiceUnavailableError(`SES sender cannot send via ${credentials.provider}`);
      }
      if (!credentials.secretKey) {
        throw new ValidationError("secretKey", "SES requires secretKey in Vault comms payload");
      }
