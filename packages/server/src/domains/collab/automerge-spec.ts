...
export function specToAutomergeDoc(spec: Record<string, unknown>): AutomergeSpecDoc {
  return Automerge.from(JSON.parse(JSON.stringify(spec)) as AutomergeSpecDoc);
}

export function automergeDocToSpec(doc: AutomergeSpecDoc): Record<string, unknown> {
  return JSON.parse(JSON.stringify(doc)) as Record<string, unknown>;
}

export function pushLocalSpec(
...
export function detectCrossParentReparent(
  ...
  const prevEls = (prev.elements ?? {}) as Record<string, { children?: string[] }>;
  const nextEls = (next.elements ?? {}) as Record<string, { children?: string[] }>;
...
function isAutomergeAssertionError(err: unknown): boolean {
  const message = err instanceof Error ? err.message : String(err);
  return message === "Assertion failed" || message.includes("Assertion failed");
}

/** Incremental apply with merge fallback when list CRDT ops assert (live collab drift). */
export function safePushLocalSpecChange(
  doc: AutomergeSpecDoc,
  prev: Record<string, unknown>,
  next: Record<string, unknown>,
): AutomergeSpecDoc {
  try {
    return pushLocalSpecChange(doc, prev, next);
  } catch (err) {
    if (!isAutomergeAssertionError(err)) throw err;
    return pushLocalSpec(doc, next);
  }
}
